...
class CartCondition
{
    ...
    {
        // If value has a percentage sign on it, we will get first its
        // percentage then we will evaluate again if the value has a minus or
        // plus sign so we can decide what to do with the percentage, whether to
        // add or subtract it to the total/subtotal/price if we can't find any
        // plus/minus sign, we will assume it as plus sign.
        ...
        // If the value has no percent sign on it, the operation will not be a
        // percentage next is we will check if it has a minus/plus sign so then
        // we can just deduct it to total/subtotal/price.
        } else {
            if ($this->valueIsToBeSubtracted($conditionValue)) {
